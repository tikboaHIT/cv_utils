...
def cutout(image, pad_size, replace=0):
    ...
    mask = np.tile(mask, [1, 1, image_channel])
    if image_channel!=3:
        replace=[128]*image_channel
    image = np.where(
        np.equal(mask, 0),
        np.ones_like(
        ...
        image)
    # if image.shape[2]!=6:
    #     print('error',image.shape,'cutout')
...
def color(image, factor):
    ...
    if image_channel==3:
        return color_ori(image, factor)
    else:
        return np.concatenate((color_ori(image[:,:,0:3], factor),color_ori(image[:,:,3:6], factor)),axis=2)
...
def rotate_ori(image, degrees, replace):
...
def rotate(image, degrees, replace):
    if image.shape[2]==3:
        return rotate_ori(image, degrees, replace)
    else:
        return np.concatenate((rotate_ori(image[:,:,0:3], degrees, replace),rotate_ori(image[:,:,3:6], degrees, replace)),axis=2)
...
def translate_x_ori(image, pixels, replace):
...
def translate_x(image, pixels, replace):
    if image.shape[2]==3:
        return translate_x_ori(image, pixels, replace)
    else:
        return np.concatenate((translate_x_ori(image[:,:,0:3], pixels, replace),translate_x_ori(image[:,:,3:6], pixels, replace)),axis=2)


def translate_y_ori(image, pixels, replace):
...
def translate_y(image, pixels, replace):
    if image.shape[2]==3:
        return translate_y_ori(image, pixels, replace)
    else:
        return np.concatenate((translate_y_ori(image[:,:,0:3], pixels, replace),translate_y_ori(image[:,:,3:6], pixels, replace)),axis=2)
...
def shear_x_ori(image, level, replace):
...
def shear_x(image, level, replace):
    if image.shape[2]==3:
        return shear_x_ori(image, level, replace)
    else:
        return np.concatenate((shear_x_ori(image[:,:,0:3], level, replace),shear_x_ori(image[:,:,3:6], level, replace)),axis=2)


def shear_y_ori(image, level, replace):
...
def shear_y(image, level, replace):
    if image.shape[2]==3:
        return shear_y_ori(image, level, replace)
    else:
        return np.concatenate((shear_y_ori(image[:,:,0:3], level, replace),shear_y_ori(image[:,:,3:6], level, replace)),axis=2)
...
def equalize(image):
    if image.shape[2]==3:
        return equalize_ori(image)
    else:
        return np.concatenate((equalize_ori(image[:,:,0:3]),equalize_ori(image[:,:,3:6])),axis=2)
